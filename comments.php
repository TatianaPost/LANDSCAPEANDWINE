...
<?php elseif (ale_get_option('comments_style') == 'wp') : ?>
	...
	<div class="blog-comments">
		<h3><span><?php _e('Comments','aletheme'); ?></span></h3>
		...
		<div id="show-hide">
			...
			<?php if (comments_open()) : ?>
				...
				<?php else : ?>
				<div class="form">
					<h4><?php _e('Leave a comment','aletheme'); ?></h4>
					<form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" id="comment-form" method="post">
						<div class="cf">
						<?php if (is_user_logged_in()) : ?>
							<p class="logged_comment_form"><?php printf(__('Logged in as <a href="%s/wp-admin/profile.php">%s</a>.', 'aletheme'), get_option('siteurl'), $user_identity); ?> <a href="<?php echo wp_logout_url(get_permalink()); ?>" title="<?php __('Log out of this account', 'aletheme'); ?>"><?php _e('Log out &raquo;', 'aletheme'); ?></a></p>
						<?php else : ?>
						<div class="left input">
							<input type="text" placeholder="<?php _e('Name', 'aletheme'); ?>" name="author" id="author" value="<?php echo esc_attr($comment_author); ?>" tabindex="1" <?php if ($req) echo "aria-required='true'"; ?> required="required">
							<input type="email" placeholder="<?php _e('E-mail', 'aletheme'); ?>" name="email" id="email" value="<?php echo esc_attr($comment_author_email); ?>" tabindex="2" <?php if ($req) echo "aria-required='true'"; ?> required="required" email="true">
						</div>
						<?php endif; ?>
						<div class="left textarea">
							<textarea name="comment" placeholder="<?php _e('Message...', 'aletheme'); ?> *" id="comment" rows="6" required="required"></textarea>
						</div>
						</div>
						<div class="c-form-footer">
							<button class="btn-com" name="submit" type="submit"><?php _e('send','aletheme'); ?></button>
						</div>
