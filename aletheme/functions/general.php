...
function ale_get_meta_gross($key, $single = true, $post_id = null) {
	if (null === $post_id) {
		$post_id = get_the_ID();
	}
	$key = 'ale_' . $key;
...
function ale_custom_page_links($cusom_query) {
	global $wp_rewrite;
	$cusom_query->query_vars['paged'] > 1 ? $current = $cusom_query->query_vars['paged'] : $current = 1;
 
	$pagination = array(
		'base' => @add_query_arg('page','%#%'),
		'format' => '',
		'total' => $cusom_query->max_num_pages,
		'current' => $current,
		'show_all' => false,
		'type' => 'list',
		'next_text' => '&raquo;',
	...
	if( !empty($cusom_query->query_vars['s']) )
		$pagination['add_args'] = array( 's' => get_query_var( 's' ) );
		...
		<div class="right">
			<div class="months">
				<?php foreach ($archives as $archive) : ?>
					...
					?>
					<div class="year-months" id="archive-year-<?php echo esc_attr($year)?>">
					<?php foreach ($months as $key => $month) :?>
						<?php foreach ($y_archives as $y_archive) :?>
							<?php if (($key == ($y_archive->month-1)) && $y_archive->posts):?>
								<a href="<?php echo get_month_link($year, $y_archive->month)?>"><?php echo esc_attr($month); ?></a>
								<?php if ($key != 11 && $delim):?>
									<span class="delim"><?php echo esc_attr($delim); ?></span>
								<?php endif;?>
								<?php break;?>
							<?php endif;?>
						<?php endforeach;?>
						<?php if ($key != $y_archive->month-1):?>
							<span><?php echo esc_attr($month); ?></span>
							<?php if ($key != 11 && $delim):?>
								<span class="delim"><?php echo esc_attr($delim); ?></span>
							<?php endif;?>
						<?php endif;?>
...
function get_breadcrumbs() {
	...
	if (is_home() || is_front_page()) {
		...
		if ( get_query_var('paged') ) {
			...
			echo '<span>→</span><span>' . __('Page') . ' ' . get_query_var('paged') . '</span>';
			if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
		}
...
/**
 * Trim content on a number of characters
 */
...
function twitt(){
	...
	if (class_exists('tp_widget_recent_tweets')) {
		...
		echo $recent_tweets->widget($args, $instance);
	...
	$fctr = '1';
	if($tp_twitter_plugin_tweets != '') {
		foreach($tp_twitter_plugin_tweets as $tweet){
			if(!empty($tweet['text'])) { ?>
				<?php echo tp_convert_links($tweet['text']); ?>
				<span class="autor"><?php _e('@','aletheme'); ?><?php echo ale_get_option('tweetlogin'); ?> <?php echo tp_relative_time($tweet['created_at']); ?></span>
				<?php
...
function woocommerce_get_product_thumbnail( ) {
	...
	if(get_the_post_thumbnail($post->ID,'product-thumba')){
		return get_the_post_thumbnail($post->ID,'product-thumba');
	} else{
		return '<img src="http://placehold.it/324x468/eeeeee/636363&amp;text=No+image" alt>';
	}
}
