		...
		<div id="aletheme-manage-sliders">		
			<ul>
				<?php foreach ($aletheme_sliders->getList() as $slider) : ?>
					...
					?>
					<li>
						<a href="<?php echo esc_url($aletheme_sliders->slidersUrl(array('action' => 'edit', 'id' => $slider->ID))) ?>">
							<span class="image">
								<?php if ($slide) : ?>
									<img src="<?php echo esc_url($slide->post_content_filtered) ?>" alt="<?php echo esc_attr($slider->post_title); ?>" />
								<?php endif; ?>
							</span>
							<span class="title"><?php echo esc_attr($slider->post_title); ?></span>
						</a>
					</li>
				<?php endforeach; ?>
			</ul>
