...
function ale_enqueue_styles() {
	...
	wp_register_style( 'aletheme_colorpicker_css', THEME_URL . '/css/colorpicker.css', array(), ALETHEME_THEME_VERSION, 'all');
	wp_register_style( 'aletheme_responsive_css', THEME_URL . '/css/responsive.css', array(), ALETHEME_THEME_VERSION, 'all');
	wp_enqueue_style('aletheme_general_css');
	if(function_exists('wc_body_class')){
		wp_enqueue_style('aletheme_woocommerce_css');
	}
	if(ale_get_option('display_color_selector')!="Disable"){
		wp_enqueue_style('aletheme_colorpicker_css');
	}
...
function ale_enqueue_scripts() {
	...
	//wp_register_script( 'cbpBGSlideshow', THEME_URL . '/js/libs/cbpBGSlideshow.js', array( 'jquery' ), ALETHEME_THEME_VERSION, true );
	//wp_register_script( 'cbpBGSlideshow.min', THEME_URL . '/js/libs/cbpBGSlideshow.min.js', array( 'jquery' ), ALETHEME_THEME_VERSION, true );
	wp_register_script( 'ale_isotope', THEME_URL . '/js/libs/isotope.js', array( 'jquery' ), ALETHEME_THEME_VERSION, true );
	...
	wp_register_script( 'ale_easydropdown', THEME_URL . '/js/libs/jquery.easydropdown.min.js', array( 'jquery' ), ALETHEME_THEME_VERSION, true );
	wp_register_script( 'ale_tweecool', THEME_URL . '/js/libs/tweecool.js', array( 'jquery' ), ALETHEME_THEME_VERSION, true );
	wp_register_script( 'ale_ui', THEME_URL . '/js/libs/jquery-ui.js', array( 'jquery' ), ALETHEME_THEME_VERSION, true );

	//wp_enqueue_script( 'cbpBGSlideshow' );
	//wp_enqueue_script( 'cbpBGSlideshow.min' );

	wp_enqueue_script( 'ale_modules' );
	wp_enqueue_script( 'ale_isotope' );
	wp_enqueue_script( 'ale_easydropdown' );
	if((ale_get_option('twiname') && is_page_template('template-home-2.php')) || (ale_get_option('twiname') && is_page_template('template-home-5.php'))){
		wp_enqueue_script( 'ale_tweecool' );
	}
	wp_enqueue_script( 'ale_ui' );
	if(ale_get_option('display_color_selector')!="Disable"){
...
<<?php echo esc_attr($tag); ?> <?php comment_class(empty( $args['has_children'] ) ? '' : 'parent') ?> id="comment-<?php comment_ID() ?>">
	...
	<?php } ?>
		...
		<div class="com-content">
			<p class="com-user"><?php printf(__('%s','aletheme'), get_comment_author_link()) ?></p>
			<span class="com-date"><?php printf( __('%1$s','aletheme'), get_comment_date(),  get_comment_time()) ?></span>
			<div class="com-text">
			...
			<?php if($depth == 1){ ?><a class="respond"><span class="icon"></span></a><?php } ?>
		</div>
		<?php if ($depth > 1) { ?>
...
<?php }
add_filter( 'wp_head', 'ale_pritty_page_load' );

//Twitter
function ale_twitt(){ ?>
	<?php if((ale_get_option('twiname') && is_page_template('template-home-2.php')) || (ale_get_option('twiname') && is_page_template('template-home-5.php'))): $twitt_count = 2;
		if(is_page_template('template-home-5.php')){
			$twitt_count = 6;
		} else{
			$twitt_count = 2;
		}
		...
		<script type="text/javascript">
			jQuery(document).ready(function ($) {
				$('.twitter-block').tweecool({
					username : '<?php echo esc_attr(ale_get_option("twiname")); ?>',
					limit : <?php echo esc_attr($twitt_count); ?>
				 });
			});
		</script>
	<?php endif; ?>
<?php }
add_action('wp_footer', 'ale_twitt');
