...
<div class="wrap" id="aletheme-edit-slider-page" data-id="<?php echo (int) $aletheme_sliders->id ?>">
	...
	<h2><?php echo esc_attr($aletheme_sliders->id) ? 'Edit' : 'Add New' ?> Slider</h2>
	...
	<div id="aletheme-edit-slider-wrapper" class="metabox-holder aletheme-slider-edit">
		...
		<div id="titlediv">
			<div id="titlewrap">						
				<input type="text" name="title" size="40" maxlength="255" placeholder="Type slider name here" required="required" id="title" value="<?php if(isset($aletheme_sliders->slider->post_title)){ echo esc_attr($aletheme_sliders->slider->post_title); } ?>" />
			</div>
		</div>
		<?php if ($aletheme_sliders->slider) : ?>
			...
			<?php if ($slider->post_status == 'publish') : ?>
				...
				<div id="aletheme-slider-info">
					...
					<dl class="shortcode" data-slug="<?php echo esc_attr($slider->post_name) ?>">
						<dt>Slider Slug</dt>
						<dd><?php echo esc_attr($slider->post_name) ?></dd>
			...
			<div id="aletheme-slides-sortable" class="meta-box-sortables ui-sortable">
					<?php foreach ($aletheme_sliders->getSlides($slider->ID) as $k => $slide) : ?>
						<div class="slide" id="aletheme-slide-<?php echo esc_attr($k) ?>" data-id="<?php echo esc_attr($slide->ID) ?>">
							<div class="handle" title="Click and drag to reorder">SORT</div>
							<a href="#" class="delete">Delete</a>
							<div class="box-image">
								<div class="image"><img /></div>
								<span><input type="text" name="image" placeholder="Or enter an image URL" value="<?php echo esc_attr($slide->post_content_filtered) ?>" /></span>
							</div>
							<div class="box-content">
								<div class="box-title"><span>Title</span><input type="text" value="<?php echo esc_attr($slide->post_title) ?>" name="title" /></div>
								<div class="box-url"><span>URL</span><input type="text" value="<?php echo esc_url($slide->pinged) ?>" name="url" /></div>
								<div class="box-description"><span>Description</span><textarea cols="30" rows="4" name="description" class="description"><?php echo esc_attr($slide->post_content) ?></textarea></div>
